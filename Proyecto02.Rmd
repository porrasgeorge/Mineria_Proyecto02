...
start_dateCR <- end_dateCR - years(3)+ days(1) ## 8 semanas atras (lunes)
```
...
sources$Name <- gsub("Coopealfaroruiz.", '', sources$Name)
...
```{r eval = FALSE}
dataLog2 <- dataLog2 %>% filter (year > 2019) %>% select(wday, hour, unbalance)
set.seed(4)
mascara <- sample.split(dataLog2$unbalance, SplitRatio = 7/10)
training_data <- dataLog2[mascara,]
test_data <- dataLog2[!mascara,]
